...
from dueling_model import DuelQNetwork
import torch
...
class Agent():
    ...
    def learn(self, experiences, gamma):
        ...
        if USING_DOUBLE_DQN:
            ...
            next_actions_local = self.qnetwork_local(next_states).max(dim=1, keepdim=True)[1]
            # Get the reward from the target model for the selected actions 
            Q_targets_next = self.qnetwork_target(next_states).gather(1, next_actions_local)
